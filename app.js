...
function SideCapsule({ screen, onOrder, onOrders, onPortfolio, badge }) {
  const items = [
    { key: 'order', icon: Send, flip: true, label: 'سفارش', onClick: onOrder, active: screen === 'order' },
    { key: 'orders', icon: ClipboardList, label: 'سفارش‌ها', onClick: onOrders, active: screen === 'orders', badge },
    { key: 'portfolio', icon: Image2, label: 'نمونه‌کار', onClick: onPortfolio, active: screen === 'portfolio' },
  ];
  return (
    <div
      style={{
        ...
        gap: 18,
        zIndex: 60,
        boxShadow: '4px 0 24px rgba(0,0,0,0.55), inset -1px 0 0 rgba(255,255,255,0.08)',
      }}
    >
      {items.map((it) => (
        <button
          key={it.key}
          onClick={it.onClick}
          style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, background: 'none', border: 'none', cursor: 'pointer', position: 'relative', padding: 2 }}
        >
          <div
            style={{
              width: 32,
              ...
              background: it.active ? colors.red : 'transparent',
            }}
          >
            <it.icon size={16} color={it.active ? '#fff' : 'rgba(255,255,255,0.6)'} style={it.flip ? { transform: 'scaleX(-1)' } : undefined} />
          </div>
          <span style={{ fontSize: 9, fontWeight: 700, color: it.active ? '#fff' : 'rgba(255,255,255,0.55)', writingMode: 'horizontal-tb' }}>{it.label}</span>
          {!!it.badge && (
            <span style={{ position: 'absolute', top: -3, right: -6, background: colors.red, color: '#fff', fontSize: 9, width: 14, height: 14, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{it.badge}</span>
          )}
        </button>
      ))}
    </div>
...
function App() {
  ...
  function goOrderFromSample(item) {
    ...
    setOrderStep(matchedService ? 2 : 1);
    setScreen('order');
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'transparent', display: 'flex', flexDirection: 'column', maxWidth: 480, margin: '0 auto', ...font }} dir="rtl">
      <SideCapsule screen={screen} onOrder={() => goOrder(null)} onOrders={() => setScreen('orders')} onPortfolio={() => setScreen('portfolio')} badge={orders.length} />
...
function PortfolioScreen({ onBack, onOrderFromSample }) {
  ...
  const categories = [{ id: 'all', title: 'همه' }, ...services.map((s) => ({ id: s.id, title: s.title }))];
