...
const colors = {
  ...
  red: '#ff3b5c',
  redDim: 'rgba(255,59,92,0.22)',
  gold: '#f0b429',
  goldDim: 'rgba(240,180,41,0.20)',
  text: '#f8f8fa',
  textMuted: '#c2c2c9',
  textFaint: '#8a8a93',
  success: '#34d399',
    ...
    <div style={{ minHeight: '100vh', background: 'transparent', display: 'flex', flexDirection: 'column', maxWidth: 480, margin: '0 auto', ...font }} dir="rtl">
      ...
      <div
        style={{
          ...
          borderRadius: 999,
          boxShadow: '0 10px 34px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.7)',
          position: 'fixed',
          left: 14,
          right: 14,
      ...
      >
        <NavItem icon={Home} label="خانه" active={screen === 'home'} onClick={() => setScreen('home')} light />
        <button
          onClick={() => goOrder(null)}
          style={{ width: 52, height: 52, borderRadius: '50%', background: `linear-gradient(135deg, ${colors.red}, #c2183a)`, border: '3px solid rgba(255,255,255,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: -34, boxShadow: '0 6px 18px rgba(255,59,92,0.55)', cursor: 'pointer' }}
        >
          <Send size={19} color="#fff" style={{ transform: 'scaleX(-1)' }} />
        </button>
        <NavItem icon={ClipboardList} label="سفارش‌ها" active={screen === 'orders'} onClick={() => setScreen('orders')} badge={orders.length} light />
      </div>
    </div>
  );
}

function NavItem({ icon: IconCmp, label, active, onClick, badge, light }) {
  const activeColor = light ? colors.red : colors.red;
  const inactiveColor = light ? '#6b6b73' : colors.textMuted;
  return (
    <button onClick={onClick} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, background: 'none', border: 'none', cursor: 'pointer', position: 'relative', color: active ? activeColor : inactiveColor, padding: '4px 16px' }}>
      <IconCmp size={21} />
      <span style={{ fontSize: 10.5, fontWeight: 700 }}>{label}</span>
      {!!badge && (
        <span style={{ position: 'absolute', top: -2, left: 6, background: colors.red, color: '#fff', fontSize: 10, width: 16, height: 16, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{badge}</span>
      )}
    </button>
  );
}
