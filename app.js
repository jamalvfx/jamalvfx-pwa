  ...
  if (active) {
    ...
    return (
      <div className="fade-up" style={{ padding: '4px 20px 20px' }}>
        ...
        <div style={{ ...glass, background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 16, overflow: 'hidden', marginBottom: 16 }}>
          {isImage && <img src={active.media_url} alt={active.title} style={{ width: '100%', display: 'block' }} />}
          {isVideo && <video src={active.media_url} controls style={{ width: '100%', display: 'block' }} />}
          {isAudio && <div style={{ padding: 16 }}><audio controls src={active.media_url} style={{ width: '100%' }} /></div>}
          ...
          return (
            <button key={item.id} onClick={() => setActive(item)} style={{ ...glass, background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 14, overflow: 'hidden', cursor: 'pointer', padding: 0, textAlign: 'right', display: 'flex', flexDirection: 'column' }}>
              <div style={{ width: '100%', aspectRatio: '1', background: colors.surface2, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                {isImage ? <img src={item.media_url} alt={item.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Image2 size={24} color={colors.textFaint} />}
              </div>
