...
function SideCapsule({ screen, onOrder, onOrders, onPortfolio, badge }) {
  const items = [
    { key: 'order', icon: Send, flip: true, onClick: onOrder, active: screen === 'order' },
    { key: 'orders', icon: ClipboardList, onClick: onOrders, active: screen === 'orders', badge },
    { key: 'portfolio', icon: Image2, onClick: onPortfolio, active: screen === 'portfolio' },
  ];
  return (
    <div
      style={{
      ...
      {items.map((it) => (
        <button
          ...
          style={{
            ...
            cursor: 'pointer',
            position: 'relative',
            boxShadow: '3px 0 14px rgba(0,0,0,0.55), inset -1px 0 0 rgba(255,255,255,0.06)',
          }}
        >
          <it.icon size={16} color={it.active ? colors.red : 'rgba(255,255,255,0.85)'} style={it.flip ? { transform: 'scaleX(-1)' } : undefined} />
          {!!it.badge && (
