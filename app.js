...
function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'transparent', display: 'flex', flexDirection: 'column', maxWidth: 480, margin: '0 auto', ...font }} dir="rtl">
      ...
      <div style={{ ...glass, display: 'flex', alignItems: 'center', justifyContent: 'space-around', padding: '10px 12px calc(14px + env(safe-area-inset-bottom))', borderTop: `1px solid ${colors.border}`, background: colors.surface, flexShrink: 0, position: 'sticky', bottom: 0 }}>
        <NavItem icon={Home} label="خانه" active={screen === 'home'} onClick={() => setScreen('home')} />
        <button
          onClick={() => goOrder(null)}
          style={{ ...glass, width: 56, height: 56, borderRadius: '50%', background: colors.red, border: `4px solid ${colors.surface}`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: -28, boxShadow: `0 6px 18px ${colors.redDim}`, cursor: 'pointer' }}
        >
