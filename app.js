...
function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: colors.bg, display: 'flex', flexDirection: 'column', maxWidth: 480, margin: '0 auto', ...font }} dir="rtl">
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 8 }}>
        {screen === 'home' && <HomeScreen onSelectService={goOrder} onOrder={() => goOrder(null)} />}
        {screen === 'order' && (
          <OrderScreen
            step={orderStep}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-around', padding: '10px 12px calc(14px + env(safe-area-inset-bottom))', borderTop: `1px solid ${colors.border}`, background: colors.surface, flexShrink: 0, position: 'sticky', bottom: 0 }}>
        ...
        <button
          ...
          style={{ width: 56, height: 56, borderRadius: '50%', background: colors.red, border: `4px solid ${colors.surface}`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: -28, boxShadow: `0 6px 18px ${colors.redDim}`, cursor: 'pointer' }}
        >
          <Send size={20} color="#fff" style={{ transform: 'scaleX(-1)' }} />
        </button>
        <NavItem icon={ClipboardList} label="سفارش‌ها" active={screen === 'orders'} onClick={() => setScreen('orders')} badge={orders.length} />
      </div>
    </div>
  );
}

function NavItem({ icon: IconCmp, label, active, onClick, badge }) {
  return (
    <button onClick={onClick} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, background: 'none', border: 'none', cursor: 'pointer', position: 'relative', color: active ? colors.red : colors.textMuted, padding: '4px 14px' }}>
      <IconCmp size={22} />
      <span style={{ fontSize: 11, fontWeight: 600 }}>{label}</span>
      {!!badge && (
        <span style={{ position: 'absolute', top: -2, left: 4, background: colors.red, color: '#fff', fontSize: 10, width: 16, height: 16, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{badge}</span>
      )}
    </button>
  );
}

function HomeScreen({ onSelectService, onOrder }) {
  return (
    <div className="fade-up" style={{ padding: '20px 20px 20px' }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${colors.surface2}, ${colors.surface})`, border: `1px solid ${colors.border}`, borderRadius: 20, padding: 20, marginBottom: 20, position: 'relative', overflow: 'hidden' }}>
        ...
        <button onClick={onOrder} style={{ background: colors.red, color: '#fff', border: 'none', borderRadius: 12, padding: '11px 22px', fontSize: 13, fontWeight: 700, cursor: 'pointer', ...font }}>
          ثبت سفارش جدید
        </button>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {services.map((s) => {
          ...
          return (
            <button key={s.id} onClick={() => onSelectService(s)} style={{ display: 'flex', alignItems: 'center', gap: 14, background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 16, padding: 14, cursor: 'pointer', textAlign: 'right', ...font }}>
    ...
    <div className="fade-up" style={{ padding: '4px 20px 20px' }}>
      ...
      {step === 1 && (
        <div>
            ...
            {services.map((s) => {
              ...
              return (
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 12, background: active ? colors.redDim : colors.surface, border: `1.5px solid ${active ? colors.red : colors.border}`, borderRadius: 14, padding: 13, cursor: 'pointer', textAlign: 'right', ...font }}
                >
          ...
          <button
            ...
            onClick={() => setStep(2)}
            style={{ width: '100%', marginTop: 22, background: selectedService ? colors.red : colors.surface2, color: selectedService ? '#fff' : colors.textFaint, border: 'none', borderRadius: 12, padding: '13px', fontWeight: 700, fontSize: 13, cursor: selectedService ? 'pointer' : 'not-allowed', ...font }}
          >
            ادامه
          </button>
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {selectedService.packages.map((p) => {
              ...
              return (
                <button key={p.id} onClick={() => setSelectedPackage(p)} style={{ background: active ? colors.redDim : colors.surface, border: `1.5px solid ${active ? colors.red : colors.border}`, borderRadius: 16, padding: 15, cursor: 'pointer', textAlign: 'right', ...font }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
            ...
            <button
              ...
              onClick={() => setStep(3)}
              style={{ ...primaryBtnStyle, flex: 1, background: selectedPackage ? colors.red : colors.surface2, color: selectedPackage ? '#fff' : colors.textFaint, cursor: selectedPackage ? 'pointer' : 'not-allowed' }}
            >
              ادامه
            </button>
          </div>
          ...
          <Field label="زمان تحویل">
            <div style={{ display: 'flex', gap: 8 }}>
              {[{ id: 'normal', label: 'عادی' }, { id: 'fast', label: 'فوری' }].map((opt) => (
                <button key={opt.id} onClick={() => updateForm('deadline', opt.id)} style={{ flex: 1, padding: '10px', borderRadius: 10, border: `1.5px solid ${form.deadline === opt.id ? colors.red : colors.border}`, background: form.deadline === opt.id ? colors.redDim : colors.surface, color: colors.text, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', ...font }}>
                  {opt.label}
                </button>
              ))}
            </div>
      ...
      {step === 4 && (
        <div>
          ...
          <div style={{ background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 16, padding: 16, display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
...
const inputStyle = { width: '100%', background: colors.surface, border: `1.5px solid ${colors.border}`, borderRadius: 10, padding: '11px 12px', color: colors.text, fontSize: 13, boxSizing: 'border-box', ...font };
const primaryBtnStyle = { background: colors.red, color: '#fff', border: 'none', borderRadius: 12, padding: '13px', fontWeight: 700, fontSize: 13, cursor: 'pointer', ...font };
const secondaryBtnStyle = { background: colors.surface, color: colors.textMuted, border: `1.5px solid ${colors.border}`, borderRadius: 12, padding: '13px 18px', fontWeight: 700, fontSize: 13, cursor: 'pointer', ...font };

function ScreenHeader({ title, onBack }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18, paddingTop: 16 }}>
      <button onClick={onBack} style={{ background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 10, width: 34, height: 34, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
