...
const colors = {
  bg: '#e4e9f5', surface: 'rgba(255,255,255,0.5)', surface2: 'rgba(255,255,255,0.68)', border: 'rgba(30,41,79,0.12)',
  red: '#ff3b5c', redDim: 'rgba(255,59,92,0.14)', gold: '#c9821a', goldDim: 'rgba(201,130,26,0.14)',
  text: '#1a1d2e', textMuted: '#5b6178', textFaint: '#8b91a8', success: '#1fa872',
};
...
const SERVICE_CATEGORIES = [
  { id: 'cover', title: 'طراحی کاور', packages: [
    { label: 'VIP', price: '۱,۱۰۰,۰۰۰ تومان' },
    { label: 'حرفه‌ای', price: '۸۸۰,۰۰۰ تومان' },
    { label: 'اقتصادی', price: '۶۵۰,۰۰۰ تومان' },
  ] },
  { id: 'equalizer', title: 'طراحی اکولایزر', packages: [
    { label: 'VIP', price: '۱,۵۰۰,۰۰۰ تومان' },
    { label: 'حرفه‌ای', price: '۱,۲۰۰,۰۰۰ تومان' },
    { label: 'اقتصادی', price: '۱,۰۰۰,۰۰۰ تومان' },
  ] },
  { id: 'visualizer', title: 'موشن موزیک', packages: [
    { label: 'اقتصادی', price: '۱,۲۰۰,۰۰۰ تومان' },
    { label: 'حرفه‌ای', price: '۱,۳۰۰,۰۰۰ تومان' },
    { label: 'VIP', price: '۱,۵۰۰,۰۰۰ تومان' },
  ] },
  { id: 'logo', title: 'طراحی لوگو', packages: [
    { label: 'VIP', price: '۲۷,۸۰۰,۰۰۰ تومان' },
    { label: 'استاندارد', price: '۱۸,۸۰۰,۰۰۰ تومان' },
  ] },
];
...
function StatusBadge({ status }) {
  const map = {
    ...
    'تحویل شده': { c: colors.success, bg: 'rgba(31,168,114,0.14)' },
  };
    ...
    <div style={{ ...glass, background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 14, padding: 16, marginBottom: 12 }}>
      ...
      {order.file_url && (
        <div style={{ marginBottom: 10 }}>
          {isImage && <img src={order.file_url} alt="عکس مرجع" style={{ maxWidth: '100%', borderRadius: 10, border: `1px solid ${colors.border}` }} />}
          {isAudio && <audio controls src={order.file_url} style={{ width: '100%' }} />}
          <div style={{ marginTop: 6 }}>
            <a href={order.file_url} target="_blank" rel="noreferrer" style={{ color: colors.red, fontSize: 12, fontWeight: 600 }}>دانلود / مشاهده عکس مرجع مشتری</a>
          </div>
        </div>
      )}
      {order.audio_file_url && (
        <div style={{ marginBottom: 10 }}>
          <audio controls src={order.audio_file_url} style={{ width: '100%' }} />
          <div style={{ marginTop: 6 }}>
            <a href={order.audio_file_url} target="_blank" rel="noreferrer" style={{ color: colors.red, fontSize: 12, fontWeight: 600 }}>دانلود فایل موزیک مشتری</a>
          </div>
        </div>
      )}
...
function PaymentBadge({ status }) {
  const map = {
    ...
    'پرداخت شده': { c: colors.success, bg: 'rgba(31,168,114,0.14)' },
  };
...
function PortfolioManager() {
  ...
  const [listFilter, setListFilter] = useState('cover');
  ...
  function startEdit(item) {
    ...
    setTitle(item.title || '');
    setCategory(item.category || 'cover');
    setPrice(item.price || '');
    setNote(item.note || '');
    setFile(null);
    setCoverImage(null);
  ...
  async function handleSave() {
    ...
    try {
      if (editingId) {
        const patch = { title: title.trim(), category, price: price || null, note: note.trim() || null };
        if (file) patch.media_url = await uploadFileToSupabase(file);
        if (coverImage) patch.cover_image_url = await uploadFileToSupabase(coverImage);
        await updatePortfolio(editingId, patch);
      } else {
        const url = await uploadFileToSupabase(file);
        const coverUrl = coverImage ? await uploadFileToSupabase(coverImage) : null;
        await insertPortfolio({ title: title.trim(), category, price: price || null, note: note.trim() || null, media_url: url, cover_image_url: coverUrl });
      }
      resetForm();
      load();
    } catch (e) {
  ...
  const currentPackages = (SERVICE_CATEGORIES.find((c) => c.id === category) || {}).packages || [];

  return (
    <div>
      <div style={{ ...glass, background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 14, padding: 16, marginBottom: 20 }}>
        <div style={{ fontWeight: 700, fontSize: 13.5, marginBottom: 12 }}>{editingId ? 'ویرایش نمونه‌کار' : 'افزودن نمونه‌کار جدید'}</div>
        <input
          value={title}
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
          {SERVICE_CATEGORIES.map((c) => (
            <button
              key={c.id}
              onClick={() => { setCategory(c.id); setPrice(''); }}
              style={{
        ...
        <div style={{ fontSize: 11, color: colors.textMuted, fontWeight: 600, marginBottom: 6 }}>قیمت مرتبط با این نمونه‌کار (اختیاری):</div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
          {currentPackages.map((p) => (
            <button
              key={p.label}
              onClick={() => setPrice(price === p.price ? '' : p.price)}
              style={{
                fontSize: 11, padding: '6px 12px', borderRadius: 8, cursor: 'pointer', fontWeight: 600,
                border: `1.5px solid ${price === p.price ? colors.success : colors.border}`,
                background: price === p.price ? 'rgba(52,211,153,0.18)' : 'transparent',
                color: price === p.price ? colors.text : colors.textMuted,
              }}
            >
              {p.label} · {p.price}
            </button>
        ...
        <label style={{ ...glass, display: 'flex', alignItems: 'center', gap: 8, background: colors.surface2, border: `1.5px dashed ${colors.border}`, borderRadius: 8, padding: '9px 12px', cursor: 'pointer', marginBottom: 8 }}>
          <span style={{ color: file ? colors.text : colors.textFaint, fontSize: 12, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {file ? file.name : editingId ? 'تغییر فایل اصلی (اختیاری)' : 'انتخاب فایل اصلی نمونه‌کار (عکس، ویدیو یا صدا)'}
          </span>
          <input type="file" accept="image/*,video/*,audio/*,.mp3,.wav,.m4a,.aac,.ogg,.flac" style={{ display: 'none' }} onChange={(e) => setFile(e.target.files && e.target.files[0] ? e.target.files[0] : null)} />
        </label>
        <label style={{ ...glass, display: 'flex', alignItems: 'center', gap: 8, background: colors.surface2, border: `1.5px dashed ${colors.border}`, borderRadius: 8, padding: '9px 12px', cursor: 'pointer', marginBottom: 10 }}>
          <span style={{ color: coverImage ? colors.text : colors.textFaint, fontSize: 12, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {coverImage ? coverImage.name : editingId ? 'تغییر تصویر کاور (اختیاری)' : 'تصویر کاور برای نمایش تو گرید (اختیاری، ولی پیشنهاد می‌شه)'}
          </span>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {editingId && (
            <button onClick={resetForm} disabled={saving} style={{ flex: 1, fontSize: 12.5, padding: '10px', borderRadius: 8, border: `1.5px solid ${colors.border}`, background: 'transparent', color: colors.textMuted, cursor: 'pointer', fontWeight: 700 }}>
              انصراف
            </button>
          )}
          <button onClick={handleSave} disabled={saving} style={{ flex: 1, fontSize: 12.5, padding: '10px', borderRadius: 8, border: 'none', background: colors.red, color: '#fff', cursor: 'pointer', fontWeight: 700, opacity: saving ? 0.6 : 1 }}>
            {saving ? 'در حال ثبت...' : editingId ? 'ذخیره تغییرات' : 'افزودن نمونه‌کار'}
          </button>
        </div>
      </div>
        ...
        {SERVICE_CATEGORIES.map((c) => {
          ...
          return (
            <button
              ...
              style={{
                fontSize: 11.5, padding: '7px 14px', borderRadius: 999, cursor: 'pointer', fontWeight: 700,
                border: `1.5px solid ${listFilter === c.id ? colors.red : colors.border}`,
                background: listFilter === c.id ? colors.redDim : 'transparent',
                color: listFilter === c.id ? colors.text : colors.textMuted,
              }}
            >
              {c.title} ({count})
            </button>
      ...
      {!loading && items.filter((i) => i.category === listFilter).length === 0 && <div style={{ color: colors.textFaint, textAlign: 'center', padding: 30, fontSize: 12.5 }}>هنوز نمونه‌کاری تو این دسته اضافه نشده</div>}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        {items.filter((i) => i.category === listFilter).map((item) => {
          ...
          return (
            <div key={item.id} style={{ ...glass, background: editingId === item.id ? colors.redDim : colors.surface, border: `1px solid ${editingId === item.id ? colors.red : colors.border}`, borderRadius: 12, overflow: 'hidden' }}>
              ...
              <div style={{ padding: 10 }}>
                ...
                <div style={{ color: colors.textFaint, fontSize: 10, marginBottom: 2 }}>{(SERVICE_CATEGORIES.find((c) => c.id === item.category) || {}).title || item.category}</div>
                {item.price && <div style={{ color: colors.success, fontSize: 10, fontWeight: 600, marginBottom: 8 }}>{item.price}</div>}
                <div style={{ display: 'flex', gap: 6, marginTop: item.price ? 0 : 8 }}>
                  <button onClick={() => startEdit(item)} style={{ flex: 1, fontSize: 10.5, padding: '6px', borderRadius: 6, border: `1.5px solid ${colors.border}`, background: 'transparent', color: colors.textMuted, cursor: 'pointer', fontWeight: 600 }}>
                    ویرایش
                  </button>
                  <button onClick={() => handleDelete(item.id)} style={{ flex: 1, fontSize: 10.5, padding: '6px', borderRadius: 6, border: `1.5px solid ${colors.red}`, background: 'transparent', color: colors.red, cursor: 'pointer', fontWeight: 600 }}>
                    حذف
                  </button>
                </div>
              </div>
